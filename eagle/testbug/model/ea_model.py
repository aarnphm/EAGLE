...
class EaModel(nn.Module):
  def __init__(self, based, draftd):
    super().__init__()
  ...
  @classmethod
  def from_pretrained(cls, based, draftd):
    model = cls(based, draftd)
  ...
  def eagenerate(
    ...
    if hasattr(self, 'tree_choices') and self.tree_choices == tree_choices:
      ...
      tree_buffers = generate_tree_buffers(tree_choices, device='cpu')
      tree_buffers['retrieve_indices_head'] = tree_buffers['retrieve_indices'].to('cpu')
    ...
    if hasattr(self, 'past_key_values'):
      ...
      (past_key_values, past_key_values_data, current_length_data) = None, None, None
  ...
  @torch.no_grad()
  def naive_generate(self, input_ids, temperature=0.0, top_p=0.0, top_k=0.0, max_steps=512, tree_choices=mc_sim_7b_63):
    if temperature > 1e-5:
      logits_processor = prepare_logits_processor(temperature=temperature, top_p=top_p, top_k=top_k)
    else:
      logits_processor = None
