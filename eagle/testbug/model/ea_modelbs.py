...
class EaModel(nn.Module):
  def __init__(self, base_model, base_model_name_or_path, ea_model_path):
    super().__init__()
  ...
  @classmethod
  def from_pretrained(cls, base_model_path=None, ea_model_path=None, **kwargs):
  ...
  def forward(
    ...
    with torch.inference_mode():
      ...
      outputs = self.base_model.model(
        input_ids=input_ids, attention_mask=attention_mask, past_key_values=past_key_values, position_ids=position_ids
      )
      if output_orig:
    ...
    if init:
      ...
      ea_logits = self.ea_layer.topK_genrate(
        hidden_states, input_ids, self.base_model.lm_head, logits_processor, attention_mask=attention_mask
      )
      if output_orig:
  ...
  def eagenerate(
    ...
    if hasattr(self, 'past_key_values') and self.past_key_values[0][0].shape[0] == bs:
      ...
      (past_key_values, past_key_values_data, current_length_data) = initialize_past_key_values(self.base_model, bs=bs)
    ...
    for idx in range(max_length):
      ...
      input_ids, tree_logits, new_token, hidden_state, sample_token, attention_mask, newfinish_flag, new_outs = (
  ...
  def naivegenerate(
    ...
    for idx in range(max_length):
      ...
      attention_mask = torch.cat(
        (attention_mask, torch.ones_like(input_id, device=attention_mask.device, dtype=attention_mask.dtype)), dim=1
      )
