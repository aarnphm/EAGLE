  ...
  if use_EaInfer:
    for output_ids in model.ea_generate(
      ...
      text = model.tokenizer.decode(
        decode_ids, skip_special_tokens=True, spaces_between_special_tokens=False, clean_up_tokenization_spaces=True
      )
    ...
    for output_ids in model.naive_generate(
      input_ids,
      temperature=temperature,
      top_p=top_p,
      max_new_tokens=args.max_new_token,
...
parser.add_argument(
  '--base-model-path',
  type=str,
  default='/home/lyh/weights/hf/vicuna_v13/13B',
...
parser.add_argument('--no-eagle3', action='store_true', help=' Not use EAGLE-3')
parser.add_argument(
  '--model-type', type=str, default='vicuna', choices=['llama-2-chat', 'vicuna', 'mixtral', 'llama-3-instruct']
)
...
with gr.Blocks(css=custom_css) as demo:
  ...
  note = gr.Markdown(
    show_label=False,
    interactive=False,
    value="""The Compression Ratio is defined as the number of generated tokens divided by the number of forward passes in the original LLM. If "Highlight the tokens generated by EAGLE-2" is checked, the tokens correctly guessed by EAGLE-2 
  ...
  regenerate_event = regenerate_button.click(
    regenerate, [chatbot, gs], [chatbot, msg, speed_box, compression_box, gs], queue=True
  ).then(
    bot, [chatbot, temperature, top_p, use_EaInfer, highlight_EaInfer, gs], [chatbot, speed_box, compression_box, gs]
  )
